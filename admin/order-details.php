...
<body>
    ...
    <div class="d-flex m-100">
        ...
        <div id="container" class="container-fluid mx-3 mt-1 p-5">
            ...
            $query = "SELECT DISTINCT item_id,item_status,item_placed,item_paid from items 
            ORDER BY
              CASE 
            ...
            ?>
                    <div class="rounded p-3 m-2" style="background-color:#D9D9D9">
                        <div class="text-success">
                            <?php
                            $l = $row['item_id'];
                            $q = "SELECT DISTINCT user_id FROM items WHERE item_id = $l";
                            $result2 = $con->query($q);
                            if ($result2->num_rows > 0) {
                                while ($row2 = $result2->fetch_assoc()) {
                                    $a = $row2['user_id'];
                                    $r = "SELECT DISTINCT user_name FROM users WHERE user_id = $a";
                                    $result1 = $con->query($r);
                                    if ($result1->num_rows > 0) {
                                        while ($row1 = $result1->fetch_assoc()) {
                                            echo "Ordered by: " . $row1['user_name'];
                                        }
                                    }
                                }
                            }
